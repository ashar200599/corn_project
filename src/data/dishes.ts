export interface ScientificName {
  ingredient: string;
  name: string;
}

export interface RecipeIngredient {
  name: string;
  quantity: number | string;
  unit: string;
}

export interface Dish {
  id: string;
  name: string;
  desc: string;
  image: string;
  emoji: string;
  country: string;
  style: 'Traditional' | 'Modern';
  category: 'Food' | 'Beverage';
  scientificNames: ScientificName[];
  ingredients?: RecipeIngredient[];
  recipe: string;
  nutrition: {
    calories: string;
    protein: string;
    carbohydrates: string;
    fat: string;
  };
  healthBenefits: string;
  excessRisks: string;
  tags?: string[];
  variations?: string[];
}

export const DISHES: Dish[] = [
  {
    id: "indo-nasi-goreng",
    variations: ["Nasi Goreng Seafood","Nasi Goreng Kambing","Nasi Goreng Gila"],
    name: "Nasi Goreng",
    ingredients: [
      {
        "name": "cooked white rice (day-old preferred)",
        "quantity": "2",
        "unit": "cups"
      },
      {
        "name": "eggs",
        "quantity": "2",
        "unit": ""
      },
      {
        "name": "garlic, minced",
        "quantity": "3",
        "unit": "cloves"
      },
      {
        "name": "shallots, minced",
        "quantity": "2",
        "unit": ""
      },
      {
        "name": "kecap manis (sweet soy sauce)",
        "quantity": "2",
        "unit": "tbsp"
      },
      {
        "name": "soy sauce",
        "quantity": "1",
        "unit": "tbsp"
      },
      {
        "name": "shrimp paste (terasi)",
        "quantity": "1",
        "unit": "tsp"
      },
      {
        "name": "red chilies, sliced",
        "quantity": "2",
        "unit": ""
      },
      {
        "name": "cooking oil",
        "quantity": "2",
        "unit": "tbsp"
      },
      {
        "name": "Salt & pepper to taste",
        "quantity": "",
        "unit": ""
      },
      {
        "name": "Toppings: fried egg, cucumber slices, prawn crackers, fried shallots",
        "quantity": "",
        "unit": ""
      }
    ],
    desc: "Classic Indonesian fried rice with sweet soy sauce, shallot, garlic, tamarind and chili.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Toppings%3A%20fried%20egg%2C%20cucumber%20slices%2C%20prawn%20crackers%2C%20fried%20shallots?width=600&height=400&nologo=true",
    emoji: "🍛",
    country: "Indonesia",
    style: "Traditional",
    category: "Food",
    scientificNames: [
      { ingredient: "Rice", name: "Oryza sativa" },
      { ingredient: "Shallot", name: "Allium cepa var. aggregatum" },
      { ingredient: "Garlic", name: "Allium sativum" },
      { ingredient: "Chili", name: "Capsicum annuum" }
    ],
    recipe: `
\n### Nasi Goreng
**Servings:** 2 | **Prep:** 10 min | **Cook:** 15 min

### Ingredients
- 2 cups cooked white rice (day-old preferred)
- 2 eggs
- 3 cloves garlic, minced
- 2 shallots, minced
- 2 tbsp kecap manis (sweet soy sauce)
- 1 tbsp soy sauce
- 1 tsp shrimp paste (terasi)
- 2 red chilies, sliced
- 2 tbsp cooking oil
- Salt & pepper to taste
- Toppings: fried egg, cucumber slices, prawn crackers, fried shallots

### Instructions
1. Heat oil in a wok over high heat.
2. Sauté garlic and shallots until fragrant (1–2 min).
3. Add shrimp paste and chili, stir for 30 seconds.
4. Push to the side, crack eggs into wok, scramble until half-cooked.
5. Add rice, mix everything together vigorously.
6. Add kecap manis and soy sauce, stir-fry for 3–4 minutes.
7. Season with salt and pepper.
8. Serve topped with fried egg, cucumber, and crackers.\n`,
    nutrition: { calories: "450 kcal", carbohydrates: "60g", protein: "12g", fat: "15g" },
    healthBenefits: "Provides essential carbohydrates for energy. Garlic and shallots offer mild antioxidant and anti-inflammatory benefits.",
    excessRisks: "Excessive consumption of carbs and saturated fats (if fried with excessive oil) can contribute to weight gain, high blood sugar, and cardiovascular strain."
  },
  {
    id: "indo-rendang",
    variations: ["Rendang Ayam","Rendang Paru","Rendang Jengkol"],
    name: "Rendang Daging",
    ingredients: [
      {
        "name": "beef (chuck or round), cut into cubes",
        "quantity": "500",
        "unit": "g"
      },
      {
        "name": "thick coconut milk",
        "quantity": "400",
        "unit": "ml"
      },
      {
        "name": "lemongrass, bruised",
        "quantity": "2",
        "unit": "stalks"
      },
      {
        "name": "kaffir lime leaves",
        "quantity": "2",
        "unit": ""
      },
      {
        "name": "**Spice paste**: Shallots, garlic, chilies, ginger, galangal, turmeric, coriander seeds.",
        "quantity": "",
        "unit": ""
      }
    ],
    desc: "Rich and tender coconut beef stew, slow-cooked with a complex spice paste.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20**Spice%20paste**%3A%20Shallots%2C%20garlic%2C%20chilies%2C%20ginger%2C%20galangal%2C%20turmeric%2C%20coriander%20seeds.?width=600&height=400&nologo=true",
    emoji: "🥩",
    country: "Indonesia",
    style: "Traditional",
    category: "Food",
    scientificNames: [
      { ingredient: "Beef", name: "Bos taurus" },
      { ingredient: "Coconut", name: "Cocos nucifera" },
      { ingredient: "Galangal", name: "Alpinia galanga" },
      { ingredient: "Lemongrass", name: "Cymbopogon citratus" }
    ],
    recipe: `

### Ingredients
* 500g beef (chuck or round), cut into cubes
* 400ml thick coconut milk
* 2 stalks lemongrass, bruised
* 2 kaffir lime leaves
* **Spice paste**: Shallots, garlic, chilies, ginger, galangal, turmeric, coriander seeds.

### Instructions
1. Blend the spice paste ingredients until smooth.
2. Sauté the paste with lemongrass and lime leaves until fragrant.
3. Add the beef cubes and stir until they change color outside.
4. Pour in the coconut milk, bring to a gentle boil.
5. Simmer on low heat for 3-4 hours, stirring occasionally until the liquid reduces completely and meat is dark brown and coated in oils.
    `,
    nutrition: { calories: "650 kcal", carbohydrates: "15g", protein: "35g", fat: "50g" },
    healthBenefits: "High protein content supports muscle growth. Turmeric and ginger have strong anti-inflammatory and antioxidant properties.",
    excessRisks: "Very high in saturated fats from coconut milk and beef, increasing the risk of hypercholesterolemia and cardiovascular diseases if consumed frequently."
  },
  {
    id: "indo-soto-ayam",
    variations: ["Soto Betawi","Soto Madura","Soto Mie"],
    name: "Soto Ayam",
    ingredients: [
      {
        "name": "chicken, cut into pieces",
        "quantity": "1",
        "unit": "whole"
      },
      {
        "name": "liters water",
        "quantity": "1.5",
        "unit": ""
      },
      {
        "name": "lemongrass, bruised",
        "quantity": "2",
        "unit": "stalks"
      },
      {
        "name": "kaffir lime leaves",
        "quantity": "3",
        "unit": ""
      },
      {
        "name": "bay leaves",
        "quantity": "2",
        "unit": ""
      },
      {
        "name": "turmeric powder",
        "quantity": "1",
        "unit": "tsp"
      },
      {
        "name": "Salt to taste",
        "quantity": "",
        "unit": ""
      }
    ],
    desc: "Yellow spicy chicken soup with turmeric, vermicelli noodles, and boiled egg.",
    image: "watermarked_img_8509685733428860263.png",
    emoji: "🍲",
    country: "Indonesia",
    style: "Traditional",
    category: "Food",
    tags: ["Main Course", "Spicy", "Soup"],
    scientificNames: [
      { ingredient: "Chicken", name: "Gallus gallus domesticus" },
      { ingredient: "Turmeric", name: "Curcuma longa" },
      { ingredient: "Cabbage", name: "Brassica oleracea var. capitata" }
    ],
    recipe: `
\n### Soto Ayam
**Servings:** 4 | **Prep:** 15 min | **Cook:** 45 min

### Ingredients
- 1 whole chicken, cut into pieces
- 1.5 liters water
- 2 stalks lemongrass, bruised
- 3 kaffir lime leaves
- 2 bay leaves
- 1 tsp turmeric powder
- Salt to taste

**Spice Paste:**
- 6 shallots
- 4 cloves garlic
- 3 cm ginger
- 3 cm galangal
- 2 cm turmeric
- 1 tsp coriander

**Toppings:**
- Vermicelli noodles (soaked)
- Boiled egg, halved
- Fried shallots
- Bean sprouts
- Lime wedges
- Sambal

### Instructions
1. Boil chicken in water with lemongrass, lime leaves, and bay leaves for 30 min.
2. Remove chicken, shred meat, set aside.
3. Sauté spice paste in oil until fragrant, add to broth.
4. Add turmeric, simmer broth for 15 more minutes. Season with salt.
5. Assemble bowls: place noodles, bean sprouts, shredded chicken in a bowl.
6. Pour hot broth over, garnish with egg, fried shallots, and lime.\n`,
    nutrition: { calories: "350 kcal", carbohydrates: "25g", protein: "30g", fat: "12g" },
    healthBenefits: "High in lean protein. Turmeric is significantly anti-inflammatory (curcumin), excellent for joint health. A hydrating and comforting broth.",
    excessRisks: "Depending on the broth reduction or added condiments (soy sauce), excessive sodium intake can lead to hypertension."
  },
  {
    id: "indo-es-campur",
    variations: ["Es Teler","Es Doger","Es Oyen"],
    name: "Es Campur",
    ingredients: [
      {
        "name": "Shaved ice",
        "quantity": "",
        "unit": ""
      },
      {
        "name": "avocado, scooped",
        "quantity": "1/2",
        "unit": ""
      },
      {
        "name": "jackfruit, sliced",
        "quantity": "1/4",
        "unit": "cup"
      },
      {
        "name": "young coconut meat",
        "quantity": "1/4",
        "unit": "cup"
      },
      {
        "name": "sweet condensed milk",
        "quantity": "2",
        "unit": "tbsp"
      },
      {
        "name": "red syrup (cocopandan)",
        "quantity": "2",
        "unit": "tbsp"
      }
    ],
    desc: "Modern Indonesian shaved ice dessert with mixed fruits, syrups, and condensed milk.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20red%20syrup%20(cocopandan)?width=600&height=400&nologo=true",
    emoji: "🍧",
    country: "Indonesia",
    style: "Modern",
    category: "Beverage",
    scientificNames: [
      { ingredient: "Avocado", name: "Persea americana" },
      { ingredient: "Jackfruit", name: "Artocarpus heterophyllus" },
      { ingredient: "Coconut", name: "Cocos nucifera" }
    ],
    recipe: `

### Ingredients
* Shaved ice
* 1/2 avocado, scooped
* 1/4 cup jackfruit, sliced
* 1/4 cup young coconut meat
* 2 tbsp sweet condensed milk
* 2 tbsp red syrup (cocopandan)

### Instructions
1. Place the fruit ingredients at the bottom of a dessert bowl.
2. Top generously with shaved ice.
3. Drizzle with red syrup and condensed milk.
4. Serve immediately and stir before eating.
    `,
    nutrition: { calories: "400 kcal", carbohydrates: "65g", protein: "4g", fat: "10g" },
    healthBenefits: "Fruits provide essential vitamins (Vitamin C, Vitamin A) and healthy fats from avocado. Coconut provides electrolytes.",
    excessRisks: "Extremely high sugar content can spike blood glucose levels, leading to an increased risk of Type 2 Diabetes and insulin resistance if consumed routinely."
  },
  {
    id: "jp-sushi",
    variations: ["Tuna Nigiri","Ebi Nigiri","Tamago Nigiri"],
    name: "Salmon Nigiri",
    ingredients: [
      {
        "name": "Sushi-grade salmon fillet",
        "quantity": "",
        "unit": ""
      },
      {
        "name": "short-grain sushi rice",
        "quantity": "1",
        "unit": "cup"
      },
      {
        "name": "rice vinegar",
        "quantity": "2",
        "unit": "tbsp"
      },
      {
        "name": "sugar, 1/2 tsp salt",
        "quantity": "1",
        "unit": "tbsp"
      }
    ],
    desc: "Vinegared rice topped with fresh, raw salmon slice.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20sugar%2C%201%2F2%20tsp%20salt?width=600&height=400&nologo=true",
    emoji: "🍣",
    country: "Japan",
    style: "Traditional",
    category: "Food",
    tags: ["Seafood", "Sushi"],
    scientificNames: [
      { ingredient: "Salmon", name: "Salmo salar" },
      { ingredient: "Rice", name: "Oryza sativa" },
      { ingredient: "Seaweed", name: "Porphyra" }
    ],
    recipe: `

### Ingredients
* Sushi-grade salmon fillet
* 1 cup short-grain sushi rice
* 2 tbsp rice vinegar
* 1 tbsp sugar, 1/2 tsp salt

### Instructions
1. Wash and cook rice. Fold in rice vinegar, sugar, and salt while rice is hot. Let it cool.
2. Slice salmon against the grain into rectangular pieces.
3. Wet hands, form a small mound of rice.
4. Place a slice of salmon on top of the rice mound and press gently.
5. Serve with soy sauce and wasabi.
    `,
    nutrition: { calories: "250 kcal (per 4 pieces)", carbohydrates: "30g", protein: "18g", fat: "7g" },
    healthBenefits: "Rich in Omega-3 fatty acids from the salmon, which promote heart health and cognitive function. Lean protein is great for recovery.",
    excessRisks: "High consumption of raw fish raises the risk of parasitic infections if not prepared properly. Frequent consumption of soy sauce drastically increases sodium intake."
  },
  {
    id: "jp-matcha",
    variations: ["Hot Matcha Latte","Matcha Espresso","Oat Milk Matcha"],
    name: "Iced Matcha Latte",
    ingredients: [
      {
        "name": "ceremonial grade matcha powder",
        "quantity": "1.5",
        "unit": "tsp"
      },
      {
        "name": "hot water (not boiling)",
        "quantity": "2",
        "unit": "oz"
      },
      {
        "name": "milk (dairy or oat)",
        "quantity": "6",
        "unit": "oz"
      },
      {
        "name": "Ice cubes",
        "quantity": "",
        "unit": ""
      },
      {
        "name": "honey or simple syrup (optional)",
        "quantity": "1",
        "unit": "tbsp"
      }
    ],
    desc: "A modern take on traditional matcha with frothy milk and ice.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20honey%20or%20simple%20syrup%20(optional)?width=600&height=400&nologo=true",
    emoji: "🍵",
    country: "Japan",
    style: "Modern",
    category: "Beverage",
    scientificNames: [
      { ingredient: "Green Tea", name: "Camellia sinensis" },
      { ingredient: "Cow's Milk", name: "Bos taurus" }
    ],
    recipe: `

### Ingredients
* 1.5 tsp ceremonial grade matcha powder
* 2 oz hot water (not boiling)
* 6 oz milk (dairy or oat)
* Ice cubes
* 1 tbsp honey or simple syrup (optional)

### Instructions
1. Sift matcha powder into a bowl.
2. Add hot water and whisk vigorously in a W-motion until frothy.
3. Fill a glass with ice, pour in the milk.
4. Top with the frothed matcha.
5. Stir and enjoy!
    `,
    nutrition: { calories: "150 kcal", carbohydrates: "12g", protein: "8g", fat: "5g" },
    healthBenefits: "Matcha is packed with L-theanine and antioxidants (EGCG), protecting cells against oxidative stress and providing calm energy.",
    excessRisks: "Overconsumption of caffeine can cause palpitations and anxiety. Sweetened versions contribute to unnecessary sugar intake."
  },
  {
    id: "it-pizza",
    variations: ["Pizza Marinara","Pizza Napoletana","Pizza Quattro Formaggi"],
    name: "Pizza Margherita",
    ingredients: [
      {
        "name": "ball of pizza dough",
        "quantity": "1",
        "unit": ""
      },
      {
        "name": "San Marzano crushed tomatoes",
        "quantity": "1/2",
        "unit": "cup"
      },
      {
        "name": "fresh mozzarella (sliced)",
        "quantity": "150",
        "unit": "g"
      },
      {
        "name": "Fresh basil leaves",
        "quantity": "",
        "unit": ""
      },
      {
        "name": "olive oil",
        "quantity": "1",
        "unit": "tbsp"
      }
    ],
    desc: "Traditional Neapolitan pizza with tomatoes, mozzarella cheese, and fresh basil.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20olive%20oil?width=600&height=400&nologo=true",
    emoji: "🍕",
    country: "Italy",
    style: "Traditional",
    category: "Food",
    scientificNames: [
      { ingredient: "Wheat", name: "Triticum" },
      { ingredient: "Tomato", name: "Solanum lycopersicum" },
      { ingredient: "Basil", name: "Ocimum basilicum" },
      { ingredient: "Cow's Milk (Cheese)", name: "Bos taurus" }
    ],
    recipe: `

### Ingredients
* 1 ball of pizza dough
* 1/2 cup San Marzano crushed tomatoes
* 150g fresh mozzarella (sliced)
* Fresh basil leaves
* 1 tbsp olive oil

### Instructions
1. Preheat oven to the highest setting (ideally with a pizza stone).
2. Stretch dough into a circle.
3. Spread tomatoes thinly across the dough.
4. Top with mozzarella slices.
5. Bake for 8-10 minutes until crust is browned and cheese is bubbling.
6. Garnish with fresh basil and a drizzle of olive oil before slicing.
    `,
    nutrition: { calories: "800 kcal (whole pizza)", carbohydrates: "90g", protein: "35g", fat: "30g" },
    healthBenefits: "Tomatoes are a great source of lycopene (good for the heart). Olive oil provides healthy monounsaturated fats.",
    excessRisks: "Excessive consumption of refined carbohydrates and cheese leads to weight gain and lethargy. High-calorie density can lead to obesity over time."
  },
  {
    id: "mx-tacos",
    variations: ["Tacos de Asada","Tacos de Carnitas","Vegan Pastor"],
    name: "Tacos al Pastor",
    ingredients: [
      {
        "name": "pork shoulder, thinly sliced",
        "quantity": "500",
        "unit": "g"
      },
      {
        "name": "pineapple, diced",
        "quantity": "1",
        "unit": "cup"
      },
      {
        "name": "Corn tortillas",
        "quantity": "",
        "unit": ""
      },
      {
        "name": "**Marinade**: Achiote paste, guajillo chilies, garlic, vinegar, oregano, cumin.",
        "quantity": "",
        "unit": ""
      },
      {
        "name": "Fresh cilantro and diced onions for garnish.",
        "quantity": "",
        "unit": ""
      }
    ],
    desc: "Traditional Mexican tacos with marinated pork, pineapple, and cilantro.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Fresh%20cilantro%20and%20diced%20onions%20for%20garnish.?width=600&height=400&nologo=true",
    emoji: "🌮",
    country: "Mexico",
    style: "Traditional",
    category: "Food",
    scientificNames: [
      { ingredient: "Pork", name: "Sus domesticus" },
      { ingredient: "Corn", name: "Zea mays" },
      { ingredient: "Pineapple", name: "Ananas comosus" },
      { ingredient: "Cilantro", name: "Coriandrum sativum" }
    ],
    recipe: `

### Ingredients
* 500g pork shoulder, thinly sliced
* 1 cup pineapple, diced
* Corn tortillas
* **Marinade**: Achiote paste, guajillo chilies, garlic, vinegar, oregano, cumin.
* Fresh cilantro and diced onions for garnish.

### Instructions
1. Blend marinade ingredients and coat the sliced pork. Marinate for at least 4 hours.
2. Cook the pork in a hot skillet until slightly charred and fully cooked.
3. Warm the corn tortillas.
4. Assemble tacos: Place pork on tortillas, top with pineapple, cilantro, and onions.
5. Serve with a squeeze of lime and hot salsa.
    `,
    nutrition: { calories: "450 kcal", carbohydrates: "35g", protein: "25g", fat: "22g" },
    healthBenefits: "Pork provides B-vitamins and zinc. Pineapple contains bromelain, aiding in digestion and reducing inflammation.",
    excessRisks: "Pork can be high in saturated fats and cholesterol. High sodium from marinades and salsas can affect blood pressure."
  },
  {
    id: "mx-horchata",
    variations: ["Horchata de Fresa","Horchata con Cafe","Dairy-Free Horchata"],
    name: "Agua de Horchata",
    ingredients: [
      {
        "name": "long-grain white rice",
        "quantity": "1",
        "unit": "cup"
      },
      {
        "name": "cinnamon stick",
        "quantity": "1",
        "unit": ""
      },
      {
        "name": "water",
        "quantity": "4",
        "unit": "cups"
      },
      {
        "name": "granulated sugar (or to taste)",
        "quantity": "1/2",
        "unit": "cup"
      },
      {
        "name": "vanilla extract",
        "quantity": "1",
        "unit": "tsp"
      },
      {
        "name": "whole milk (optional)",
        "quantity": "1/2",
        "unit": "cup"
      }
    ],
    desc: "Refreshing traditional Mexican rice-based beverage flavored with cinnamon and vanilla.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20whole%20milk%20(optional)?width=600&height=400&nologo=true",
    emoji: "🥤",
    country: "Mexico",
    style: "Traditional",
    category: "Beverage",
    scientificNames: [
      { ingredient: "Rice", name: "Oryza sativa" },
      { ingredient: "Cinnamon", name: "Cinnamomum verum" },
      { ingredient: "Vanilla", name: "Vanilla planifolia" }
    ],
    recipe: `

### Ingredients
* 1 cup long-grain white rice
* 1 cinnamon stick
* 4 cups water
* 1/2 cup granulated sugar (or to taste)
* 1 tsp vanilla extract
* 1/2 cup whole milk (optional)

### Instructions
1. Rinse the rice. Soak rice and cinnamon stick in 4 cups of water overnight.
2. Blend the rice, cinnamon, and soaking water until smooth.
3. Strain the mixture through a fine mesh sieve or cheesecloth into a pitcher.
4. Stir in sugar, vanilla, and milk until dissolved.
5. Serve chilled over ice with a sprinkle of ground cinnamon.
    `,
    nutrition: { calories: "210 kcal", carbohydrates: "40g", protein: "3g", fat: "4g" },
    healthBenefits: "Cinnamon has anti-inflammatory properties and may help regulate blood sugar levels. A cooling and hydrating drink.",
    excessRisks: "Added sugars can significantly increase calorie intake, increasing the risk of metabolic syndrome and insulin resistance."
  },
  {
    id: "th-pad-thai",
    variations: ["Pad Thai with Chicken","Pad Thai with Tofu","Vegetarian Pad Thai"],
    name: "Pad Thai",
    ingredients: [
      {
        "name": "dried flat rice noodles",
        "quantity": "200",
        "unit": "g"
      },
      {
        "name": "shrimp, peeled and deveined",
        "quantity": "150",
        "unit": "g"
      },
      {
        "name": "eggs, lightly beaten",
        "quantity": "2",
        "unit": ""
      },
      {
        "name": "bean sprouts",
        "quantity": "1",
        "unit": "cup"
      },
      {
        "name": "crushed roasted peanuts",
        "quantity": "1/4",
        "unit": "cup"
      },
      {
        "name": "**Sauce**: Tamarind paste, fish sauce, palm sugar.",
        "quantity": "",
        "unit": ""
      }
    ],
    desc: "Classic Thai stir-fried rice noodles with eggs, peanuts, bean sprouts, and lime.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20**Sauce**%3A%20Tamarind%20paste%2C%20fish%20sauce%2C%20palm%20sugar.?width=600&height=400&nologo=true",
    emoji: "🍜",
    country: "Thailand",
    style: "Traditional",
    category: "Food",
    scientificNames: [
      { ingredient: "Rice (Noodles)", name: "Oryza sativa" },
      { ingredient: "Peanut", name: "Arachis hypogaea" },
      { ingredient: "Shrimp", name: "Caridea" },
      { ingredient: "Mung Bean (Sprouts)", name: "Vigna radiata" }
    ],
    recipe: `

### Ingredients
* 200g dried flat rice noodles
* 150g shrimp, peeled and deveined
* 2 eggs, lightly beaten
* 1 cup bean sprouts
* 1/4 cup crushed roasted peanuts
* **Sauce**: Tamarind paste, fish sauce, palm sugar.

### Instructions
1. Soak rice noodles in warm water until pliable, then drain.
2. Mix the sauce ingredients (tamarind, fish sauce, sugar) in a small bowl.
3. Heat oil in a wok. Cook the shrimp until pink, then remove.
4. Add eggs to the wok and scramble lightly. Add noodles and sauce, tossing constantly.
5. Add bean sprouts and cooked shrimp, tossing until noodles are cooked and coated.
6. Serve immediately garnished with crushed peanuts and a lime wedge.
    `,
    nutrition: { calories: "550 kcal", carbohydrates: "70g", protein: "25g", fat: "20g" },
    healthBenefits: "Shrimp is a great source of lean protein and iodine. Bean sprouts provide vitamin C and dietary fiber.",
    excessRisks: "Can be high in sodium due to fish sauce, and high in sugar if Palm sugar is overused. Peanut oil and peanuts elevate caloric density."
  },
  {
    id: "in-tikka-masala",
    variations: ["Paneer Tikka Masala","Mutton Tikka Masala","Vegan Tikka Masala"],
    name: "Chicken Tikka Masala",
    ingredients: [
      {
        "name": "boneless chicken thigh, cut into pieces",
        "quantity": "500",
        "unit": "g"
      },
      {
        "name": "**Marinade**: Yogurt, lemon juice, turmeric, garam masala, cumin.",
        "quantity": "",
        "unit": ""
      },
      {
        "name": "**Sauce**: Onion, garlic, ginger, tomato purée, heavy cream, cumin, paprika.",
        "quantity": "",
        "unit": ""
      }
    ],
    desc: "Roasted marinated chicken chunks in a spiced modern Indian curry sauce.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20**Sauce**%3A%20Onion%2C%20garlic%2C%20ginger%2C%20tomato%20pur%C3%A9e%2C%20heavy%20cream%2C%20cumin%2C%20paprika.?width=600&height=400&nologo=true",
    emoji: "🥘",
    country: "India",
    style: "Modern",
    category: "Food",
    scientificNames: [
      { ingredient: "Chicken", name: "Gallus gallus domesticus" },
      { ingredient: "Tomato", name: "Solanum lycopersicum" },
      { ingredient: "Coriander", name: "Coriandrum sativum" }
    ],
    recipe: `

### Ingredients
* 500g boneless chicken thigh, cut into pieces
* **Marinade**: Yogurt, lemon juice, turmeric, garam masala, cumin.
* **Sauce**: Onion, garlic, ginger, tomato purée, heavy cream, cumin, paprika.

### Instructions
1. Marinate chicken for at least 1 hour. Broil or grill until slightly charred.
2. For the sauce, sauté onions, garlic, and ginger until soft. Add spices and toast for 1 min.
3. Pour in tomato purée and simmer for 15 minutes.
4. Stir in heavy cream and add the cooked chicken pieces.
5. Simmer for another 5-10 minutes. Garnish with fresh coriander and serve with naan.
    `,
    nutrition: { calories: "600 kcal", carbohydrates: "20g", protein: "35g", fat: "42g" },
    healthBenefits: "Chicken provides high-quality protein. Spices like turmeric, ginger, and garlic offer immense antioxidant value.",
    excessRisks: "The heavy cream used in the sauce adds significant saturated fat, increasing cholesterol and cardiovascular disease risks."
  },
  {
    id: "kr-kimchi",
    variations: ["Pork Belly Kimchi Jjigae","Tuna Kimchi Jjigae","Spam Kimchi Jjigae"],
    name: "Kimchi Jjigae",
    ingredients: [
      {
        "name": "well-fermented (sour) kimchi, chopped",
        "quantity": "2",
        "unit": "cups"
      },
      {
        "name": "pork belly or shoulder, sliced",
        "quantity": "200",
        "unit": "g"
      },
      {
        "name": "block firm tofu, sliced",
        "quantity": "1/2",
        "unit": ""
      },
      {
        "name": "onion, sliced",
        "quantity": "1/2",
        "unit": ""
      },
      {
        "name": "scallions, chopped",
        "quantity": "2",
        "unit": ""
      },
      {
        "name": "gochugaru (Korean chili flakes)",
        "quantity": "1",
        "unit": "tbsp"
      },
      {
        "name": "water or anchovy-kelp stock",
        "quantity": "4",
        "unit": "cups"
      }
    ],
    desc: "A hearty traditional Korean stew made with aged kimchi, tofu, and pork.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20water%20or%20anchovy-kelp%20stock?width=600&height=400&nologo=true",
    emoji: "🍲",
    country: "South Korea",
    style: "Traditional",
    category: "Food",
    scientificNames: [
      { ingredient: "Napa Cabbage (Kimchi)", name: "Brassica rapa subsp. pekinensis" },
      { ingredient: "Soybean (Tofu)", name: "Glycine max" },
      { ingredient: "Pork", name: "Sus domesticus" }
    ],
    recipe: `

### Ingredients
* 2 cups well-fermented (sour) kimchi, chopped
* 200g pork belly or shoulder, sliced
* 1/2 block firm tofu, sliced
* 1/2 onion, sliced
* 2 scallions, chopped
* 1 tbsp gochugaru (Korean chili flakes)
* 4 cups water or anchovy-kelp stock

### Instructions
1. In a pot, sauté the pork and kimchi together until the pork is no longer pink over medium heat.
2. Add onion and gochugaru, stir well.
3. Pour in the stock/water, bring to a boil. Reduce heat and let it rapidly simmer for 15-20 minutes.
4. Place the tofu slices on top, cook for another 5 minutes.
5. Garnish with chopped scallions and serve hot with rice.
    `,
    nutrition: { calories: "380 kcal", carbohydrates: "15g", protein: "22g", fat: "25g" },
    healthBenefits: "Kimchi is loaded with probiotics essential for gut health. Tofu provides plant-based protein and isoflavones.",
    excessRisks: "Kimchi can be extremely high in sodium, which is a risk factor for high blood pressure and stomach inflammation/ulcers."
  },
  {
    id: "fr-croissant",
    variations: ["Almond Croissant","Chocolate Croissant (Pain au Chocolat)","Ham & Cheese Croissant"],
    name: "Butter Croissant",
    ingredients: [
      {
        "name": "all-purpose flour",
        "quantity": "500",
        "unit": "g"
      },
      {
        "name": "salt, 50g sugar",
        "quantity": "10",
        "unit": "g"
      },
      {
        "name": "instant yeast",
        "quantity": "10",
        "unit": "g"
      },
      {
        "name": "water, 140ml whole milk",
        "quantity": "140",
        "unit": "ml"
      },
      {
        "name": "unsalted butter, cold (for laminating)",
        "quantity": "250",
        "unit": "g"
      },
      {
        "name": "egg (for egg wash)",
        "quantity": "1",
        "unit": ""
      }
    ],
    desc: "A flaky, buttery modern French pastry with a classic crescent shape.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20egg%20(for%20egg%20wash)?width=600&height=400&nologo=true",
    emoji: "🥐",
    country: "France",
    style: "Modern",
    category: "Food",
    scientificNames: [
      { ingredient: "Wheat", name: "Triticum" },
      { ingredient: "Cow's Milk (Butter)", name: "Bos taurus" }
    ],
    recipe: `

### Ingredients
* 500g all-purpose flour
* 10g salt, 50g sugar
* 10g instant yeast
* 140ml water, 140ml whole milk
* 250g unsalted butter, cold (for laminating)
* 1 egg (for egg wash)

### Instructions
1. Make the dough (flour, salt, sugar, yeast, milk, water) and let it rest in the fridge overnight.
2. Prepare a butter block by pounding the cold butter into a square.
3. Enclose the butter block in the dough, and perform a series of rolls and folds (laminating) to create layers, chilling between folds.
4. Roll out the laminated dough, cut into triangles, and roll into croissants.
5. Proof until doubled in size, brush with egg wash, and bake at 200°C for 15-20 minutes until golden.
    `,
    nutrition: { calories: "400 kcal", carbohydrates: "38g", protein: "6g", fat: "22g" },
    healthBenefits: "Provides quick energy from carbohydrates. Dairy fat provides fat-soluble vitamins like Vitamin A.",
    excessRisks: "High in refined carbohydrates and saturated fats. Lacks dietary fiber. Overconsumption contributes to obesity and heart disease."
  },
  {
    id: "us-avocado-toast",
    variations: ["Avocado Toast with Egg","Avocado Toast with Smoked Salmon","Spicy Avocado Toast"],
    name: "Avocado Toast",
    ingredients: [
      {
        "name": "thick-cut sourdough bread",
        "quantity": "1",
        "unit": "slice"
      },
      {
        "name": "ripe avocado",
        "quantity": "1/2",
        "unit": ""
      },
      {
        "name": "lemon juice",
        "quantity": "1",
        "unit": "tsp"
      },
      {
        "name": "Pinch of salt and red pepper flakes",
        "quantity": "",
        "unit": ""
      },
      {
        "name": "Optional: 1 poached egg, seeds (chia/sesame)",
        "quantity": "",
        "unit": ""
      }
    ],
    desc: "Modern café breakfast featuring mashed avocado on sourdough toast, topped with seeds.",
    image: "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Optional%3A%201%20poached%20egg%2C%20seeds%20(chia%2Fsesame)?width=600&height=400&nologo=true",
    emoji: "🥑",
    country: "USA",
    style: "Modern",
    category: "Food",
    scientificNames: [
      { ingredient: "Avocado", name: "Persea americana" },
      { ingredient: "Wheat (Sourdough)", name: "Triticum" },
      { ingredient: "Chicken (Egg)", name: "Gallus gallus domesticus" }
    ],
    recipe: `

### Ingredients
* 1 slice thick-cut sourdough bread
* 1/2 ripe avocado
* 1 tsp lemon juice
* Pinch of salt and red pepper flakes
* Optional: 1 poached egg, seeds (chia/sesame)

### Instructions
1. Toast the sourdough bread until golden and crisp.
2. In a small bowl, roughly mash the avocado with lemon juice and salt.
3. Spread the mashed avocado over the toast.
4. Top with red pepper flakes, seeds, and optionally a poached egg.
5. Serve immediately.
    `,
    nutrition: { calories: "350 kcal", carbohydrates: "30g", protein: "12g", fat: "22g" },
    healthBenefits: "Avocados are rich in monounsaturated fats (oleic acid), which are heart-healthy. Sourdough provides a good source of gut-friendly prebiotics.",
    excessRisks: "While they are healthy fats, avocados are calorie-dense. Unconscious overconsumption can lead to caloric surplus and weight gain."
  }
  ,
  {
  "id": "indo-gado-gado",
  "name": "Gado-Gado",
  "desc": "Indonesian salad of slightly boiled, blanched or steamed vegetables and hard-boiled eggs, boiled potato, fried tofu and tempeh, served with a peanut sauce dressing.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Gado-Gado?width=600&height=400&nologo=true",
  "emoji": "🥗",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Peanut",
      "name": "Arachis hypogaea"
    },
    {
      "ingredient": "Soybean (Tofu/Tempeh)",
      "name": "Glycine max"
    },
    {
      "ingredient": "Cabbage",
      "name": "Brassica oleracea var. capitata"
    },
    {
      "ingredient": "Potato",
      "name": "Solanum tuberosum"
    }
  ],
  "recipe": `\n### Ingredients\n* 100g cabbage, shredded and blanched\n* 100g spinach, blanched\n* 100g bean sprouts, blanched\n* 1 potato, boiled and cubed\n* 1 block tofu & tempeh, fried and cubed\n* 2 hard-boiled eggs, halved\n* **Peanut Sauce**: 200g fried peanuts, 3 cloves garlic, 2 bird's eye chilies, 1 tbsp tamarind juice, 2 tbsp palm sugar, salt, water.\n\n### Instructions\n1. Blend or pound the fried peanuts, garlic, chilies, and palm sugar into a paste.\n2. Add tamarind juice and water to achieve a smooth, pourable consistency. Season with salt.\n3. Arrange the blanched vegetables, potato, tofu, tempeh, and eggs on a plate.\n4. Generously pour the peanut sauce over the assembled ingredients.\n5. Garnish with fried shallots and serve with prawn crackers (krupuk).\n    `,
  "nutrition": { calories: "350 kcal", carbohydrates: "30g", protein: "18g", fat: "20g" },
  "healthBenefits": "Rich in plant-based proteins (tofu, tempeh) and essential vitamins from fresh vegetables. Peanuts provide healthy fats.",
  "excessRisks": "Peanut sauce is calorie and fat-dense. Overconsumption can lead to weight gain."
},
  {
  "id": "indo-sate-ayam",
  "name": "Sate Ayam & Kambing",
  "desc": "Grilled skewered meat served with peanut sauce (chicken) or sweet soy sauce (goat).",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Sate%20Ayam%20%26%20Kambing?width=600&height=400&nologo=true",
  "emoji": "🍢",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chicken",
      "name": "Gallus gallus domesticus"
    },
    {
      "ingredient": "Goat",
      "name": "Capra hircus"
    },
    {
      "ingredient": "Soybean (Soy Sauce)",
      "name": "Glycine max"
    }
  ],
  "recipe": `\n### Ingredients\n* 500g chicken or goat meat, cut into bite-sized cubes\n* Bamboo skewers (soaked in water)\n* **Marinade**: 2 tbsp sweet soy sauce, 1 tbsp oil, 1 tsp coriander powder.\n* **Peanut Sauce (for chicken)**: Ground peanuts, sweet soy sauce, garlic, chili, water.\n\n### Instructions\n1. Marinate the meat cubes for at least 30 minutes.\n2. Thread 4-5 pieces of meat onto each bamboo skewer.\n3. Grill over hot charcoal, turning frequently and brushing with extra marinade until cooked and slightly charred.\n4. Serve chicken sate with hot peanut sauce, and goat sate with sweet soy sauce mixed with chopped shallots and chilies.\n    `,
  "nutrition": { calories: "400 kcal", carbohydrates: "15g", protein: "35g", fat: "22g" },
  "healthBenefits": "Excellent source of high-quality protein and iron, especially from goat meat. Grilling without excess oil keeps it relatively lean.",
  "excessRisks": "Charred meats contain PAHs and HCAs which are potential carcinogens. High consumption of sweet soy sauce increases sugar intake."
},
  {
  "id": "indo-opor-ayam",
  "name": "Opor Ayam",
  "desc": "Chicken braised in coconut milk with traditional Indonesian spices, commonly served during Eid.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Opor%20Ayam?width=600&height=400&nologo=true",
  "emoji": "🍲",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chicken",
      "name": "Gallus gallus domesticus"
    },
    {
      "ingredient": "Coconut",
      "name": "Cocos nucifera"
    },
    {
      "ingredient": "Coriander",
      "name": "Coriandrum sativum"
    }
  ],
  "recipe": `\n### Ingredients\n* 1 whole chicken, cut into parts\n* 500ml thin coconut milk\n* 200ml thick coconut milk\n* 2 bay leaves, 2 lemongrass stalks\n* **Spice Paste**: Shallots, garlic, coriander, cumin, galangal, turmeric (optional for white opor).\n\n### Instructions\n1. Sauté the spice paste with bay leaves and lemongrass until fragrant.\n2. Add the chicken pieces and cook until the outside turns opaque.\n3. Pour in the thin coconut milk and simmer until the chicken is tender.\n4. Stir in the thick coconut milk, season with salt and a pinch of sugar.\n5. Simmer gently for another 10 minutes without letting it boil rapidly. Serve with ketupat or rice.\n    `,
  "nutrition": { calories: "450 kcal", carbohydrates: "8g", protein: "30g", fat: "35g" },
  "healthBenefits": "Provides sustained energy and good protein. Spices offer anti-inflammatory properties.",
  "excessRisks": "High saturated fat content from coconut milk can increase LDL cholesterol levels if consumed frequently in large portions."
},
  {
  "id": "indo-rawon",
  "name": "Rawon",
  "desc": "Rich tasting traditional Indonesian beef black soup. The dark color comes from the earthy keluak nut.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Rawon?width=600&height=400&nologo=true",
  "emoji": "🍲",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Beef",
      "name": "Bos taurus"
    },
    {
      "ingredient": "Keluak Nut",
      "name": "Pangium edule"
    },
    {
      "ingredient": "Mung Bean (Sprouts)",
      "name": "Vigna radiata"
    }
  ],
  "recipe": `\n### Ingredients\n* 500g beef brisket, cubed\n* 6 keluak nuts (crack open, soak inner flesh in warm water)\n* 2 lemongrass stalks, 4 kaffir lime leaves\n* **Spice Paste**: Shallots, garlic, turmeric, ginger, coriander.\n* Short beansprouts, salted egg, shrimp crackers (for serving).\n\n### Instructions\n1. Boil beef until tender, reserve the broth and cut the meat into smaller cubes.\n2. Blend the soaked keluak flesh with the other spice paste ingredients.\n3. Sauté the paste with lemongrass and lime leaves until fragrant and dark.\n4. Add the sautéed paste into the beef broth along with the meat.\n5. Simmer for 30 minutes to let flavors meld. Serve hot topped with raw short beansprouts.\n    `,
  "nutrition": { calories: "400 kcal", carbohydrates: "10g", protein: "30g", fat: "25g" },
  "healthBenefits": "High in protein and iron from the beef. Keluak contains oleic acid (healthy fat).",
  "excessRisks": "Keluak nuts are toxic if not fermented properly. High purine content in beef brisket can trigger gout in susceptible individuals."
},
  {
  "id": "indo-pempek",
  "name": "Pempek",
  "desc": "Savory Indonesian fishcake delicacy, made of fish and tapioca, served with rich sweet and sour sauce.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Pempek?width=600&height=400&nologo=true",
  "emoji": "🥟",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Mackerel Fish",
      "name": "Scomberomorus"
    },
    {
      "ingredient": "Cassava (Tapioca)",
      "name": "Manihot esculenta"
    },
    {
      "ingredient": "Tamarind",
      "name": "Tamarindus indica"
    }
  ],
  "recipe": `\n### Ingredients\n* 500g Spanish Mackerel fillet, finely ground\n* 300g tapioca starch\n* 1 egg, 2 cloves garlic (mashed), salt\n* **Cuko (Sauce)**: 250g palm sugar, 50g tamarind, garlic, bird's eye chilies, vinegar, water.\n\n### Instructions\n1. Mix ground fish, garlic, salt, and egg until sticky. Gradually fold in tapioca starch until moldable.\n2. Shape the dough into cylinders (lenjer) or stuff with beaten eggs (kapal selam).\n3. Boil the shaped dough in salted water until they float. Remove and drain.\n4. Deep fry the boiled fishcakes until golden brown.\n5. Boil cuko ingredients until sugar dissolves, strain. Serve fried pempek with the cuko sauce.\n    `,
  "nutrition": { calories: "350 kcal", carbohydrates: "45g", protein: "15g", fat: "12g" },
  "healthBenefits": "Fish provides excellent Omega-3 fatty acids and lean protein. Tapioca is gluten-free.",
  "excessRisks": "Deep frying adds empty calories and unhealthy fats. The 'cuko' sauce is extremely high in sugar."
},
  {
  "id": "indo-bakso",
  "name": "Bakso",
  "desc": "Indonesian meatball soup served with noodles, tofu, and a savory beef broth.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Bakso?width=600&height=400&nologo=true",
  "emoji": "🍲",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Beef",
      "name": "Bos taurus"
    },
    {
      "ingredient": "Wheat (Noodles)",
      "name": "Triticum"
    },
    {
      "ingredient": "Garlic",
      "name": "Allium sativum"
    }
  ],
  "recipe": `\n### Ingredients\n* **Meatballs**: 500g finely ground beef, 50g tapioca starch, ice cubes, garlic powder, salt, baking powder.\n* **Broth**: Beef bones, water, garlic, celery, salt, white pepper.\n* Egg noodles, vermicelli, bok choy for assembly.\n\n### Instructions\n1. Blend ground beef, tapioca, ice cubes, garlic powder, and baking powder in a food processor until it forms a smooth, bouncy paste.\n2. Shape into balls and drop into hot (not boiling) water until they float.\n3. For the broth, simmer beef bones with bruised garlic and celery stalks for 2-3 hours.\n4. Assemble noodles and bok choy in a bowl, top with meatballs, and pour boiling broth over.\n5. Garnish with fried shallots and celery leaves.\n    `,
  "nutrition": { calories: "450 kcal", carbohydrates: "40g", protein: "25g", fat: "20g" },
  "healthBenefits": "Provides a good mix of carbs and protein. A comforting meal that aids hydration.",
  "excessRisks": "Commercially made meatballs may contain excessive MSG and sodium. High sodium can lead to hypertension."
},
  {
  "id": "indo-nasi-padang",
  "name": "Nasi Padang",
  "desc": "Steamed rice served with various pre-cooked dishes from West Sumatra.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Nasi%20Padang?width=600&height=400&nologo=true",
  "emoji": "🍛",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Rice",
      "name": "Oryza sativa"
    },
    {
      "ingredient": "Cassava Leaves",
      "name": "Manihot esculenta"
    },
    {
      "ingredient": "Jackfruit",
      "name": "Artocarpus heterophyllus"
    }
  ],
  "recipe": `\n### Ingredients\n* Steamed white rice\n* Boiled young cassava leaves\n* Green chili sambal (Sambal Ijo)\n* Gulai Nangka (Young jackfruit curry)\n* Choice of protein: Rendang, Ayam Pop, or Ayam Bakar.\n\n### Instructions\n1. Nasi Padang is an assembly of dishes rather than a single recipe.\n2. Cook rice and place a generous portion on a plate.\n3. Add a scoop of young jackfruit curry and a side of boiled cassava leaves.\n4. Add a spoonful of green chili sambal.\n5. Top with your chosen protein dish (e.g., Rendang) and drizzle with extra curry sauce.\n    `,
  "nutrition": { calories: "700+ kcal", carbohydrates: "80g", protein: "25g", fat: "35g" },
  "healthBenefits": "Cassava leaves are rich in iron and vitamins. Spices provide broad antioxidant coverage.",
  "excessRisks": "Highly calorically dense due to coconut milk in almost all accompanying dishes. Regular consumption without portion control leads to obesity and high cholesterol."
},
  {
  "id": "indo-gudeg",
  "name": "Gudeg",
  "desc": "A traditional Javanese dish made from young unripe jackfruit stewed for hours in palm sugar and coconut milk.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Gudeg?width=600&height=400&nologo=true",
  "emoji": "🍲",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Young Jackfruit",
      "name": "Artocarpus heterophyllus"
    },
    {
      "ingredient": "Coconut",
      "name": "Cocos nucifera"
    },
    {
      "ingredient": "Teak Leaves",
      "name": "Tectona grandis"
    }
  ],
  "recipe": `\n### Ingredients\n* 500g young jackfruit, chopped\n* 500ml coconut milk\n* 100g dark palm sugar\n* Teak leaves (gives the reddish-brown color)\n* **Spices**: Coriander, shallots, garlic, candlenut, galangal, bay leaves.\n\n### Instructions\n1. Blend shallots, garlic, coriander, and candlenut into a paste.\n2. Line the bottom of a heavy pot with teak leaves. Add the jackfruit, spice paste, palm sugar, and galangal.\n3. Pour in coconut milk to cover.\n4. Simmer on very low heat for 3 to 4 hours until the liquid has completely evaporated, the jackfruit is tender, and turns reddish-brown.\n5. Serve with rice, thick coconut cream (areh), and spicy cow skin stew (krecek).\n    `,
  "nutrition": { calories: "350 kcal", carbohydrates: "45g", protein: "5g", fat: "15g" },
  "healthBenefits": "Jackfruit is a good source of fiber and potassium. The slow-cooking process breaks down complex fibers making it easy to digest.",
  "excessRisks": "High sugar content from palm sugar makes this an extremely sweet dish, risky for diabetics."
},
  {
  "id": "indo-nasi-liwet",
  "name": "Nasi Liwet",
  "desc": "Succulent rice dish cooked in coconut milk, chicken broth, and spices.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Nasi%20Liwet?width=600&height=400&nologo=true",
  "emoji": "🍚",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Rice",
      "name": "Oryza sativa"
    },
    {
      "ingredient": "Anchovy",
      "name": "Engraulidae"
    },
    {
      "ingredient": "Lemongrass",
      "name": "Cymbopogon citratus"
    }
  ],
  "recipe": `\n### Ingredients\n* 2 cups jasmine rice\n* 1 cup thin coconut milk, 1 cup chicken broth\n* 2 stalks lemongrass, 3 bay leaves\n* 3 shallots, sliced\n* Handful of fried salted anchovies (teri medan)\n\n### Instructions\n1. Wash rice and place in a rice cooker or heavy-bottomed pot.\n2. Add coconut milk, chicken broth, sliced shallots, lemongrass, and bay leaves.\n3. Season lightly with salt (anchovies will add saltiness later).\n4. Cook until done. Fluff the rice.\n5. Top with fried anchovies before serving. Often accompanied by shredded chicken and chayote stew.\n    `,
  "nutrition": { calories: "450 kcal", carbohydrates: "55g", protein: "10g", fat: "18g" },
  "healthBenefits": "Anchovies are tiny nutritional powerhouses offering calcium and omega-3s. Comforting and easily digestible.",
  "excessRisks": "Calorie-dense due to coconut milk. Salted anchovies provide high sodium, which can increase blood pressure."
},
  {
  "id": "indo-pecel-lele",
  "name": "Pecel Lele",
  "desc": "Deep-fried catfish served with traditional sambal and fresh vegetables.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Pecel%20Lele?width=600&height=400&nologo=true",
  "emoji": "🐟",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Catfish",
      "name": "Clarias"
    },
    {
      "ingredient": "Tomato",
      "name": "Solanum lycopersicum"
    },
    {
      "ingredient": "Basil (Lemon Basil)",
      "name": "Ocimum × africanum"
    }
  ],
  "recipe": `\n### Ingredients\n* 2 whole catfish, cleaned and scored\n* **Marinade**: Turmeric, coriander, garlic, salt, lime juice.\n* **Sambal**: Tomatoes, shallots, garlic, bird's eye chilies, shrimp paste (terasi), salt, sugar.\n* Fresh cabbage, cucumber, lemon basil for serving.\n\n### Instructions\n1. Marinate the catfish for 15 minutes.\n2. Deep fry the catfish in hot oil until crispy and golden brown.\n3. For the sambal: lightly fry the tomatoes, shallots, garlic, and chilies until soft. Grind them in a mortar with toasted shrimp paste, salt, and sugar.\n4. Serve the crispy catfish with rice, sambal, and fresh raw vegetables (lalapan).\n    `,
  "nutrition": { calories: "500 kcal", carbohydrates: "15g", protein: "35g", fat: "30g" },
  "healthBenefits": "Catfish is a healthy lean protein and contains B12. Raw vegetables provide valuable enzymes and fiber.",
  "excessRisks": "Deep frying in reused oil increases trans fats and free radicals. High frequent consumption can harm cardiovascular health."
},
  {
  "id": "indo-ketoprak",
  "name": "Ketoprak",
  "desc": "Vegetarian dish consisting of tofu, vegetables, rice cake, and rice vermicelli served in peanut sauce.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Ketoprak?width=600&height=400&nologo=true",
  "emoji": "🍝",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Peanut",
      "name": "Arachis hypogaea"
    },
    {
      "ingredient": "Garlic",
      "name": "Allium sativum"
    },
    {
      "ingredient": "Soybean (Tofu)",
      "name": "Glycine max"
    }
  ],
  "recipe": `\n### Ingredients\n* 1 rice cake (lontong or ketupat), sliced\n* 1 block fried tofu, cubed\n* 50g rice vermicelli (bihun), softened\n* Handful of beansprouts, blanched\n* **Peanut Sauce**: Ground fried peanuts, garlic, bird's eye chili, sweet soy sauce, water.\n* Fried shallots and crackers.\n\n### Instructions\n1. Prepare the peanut sauce by grinding garlic and chili, then mixing with ground peanuts, sweet soy sauce, and enough water to make a thick dressing.\n2. On a plate, arrange the rice cake, vermicelli, tofu, and beansprouts.\n3. Pour the peanut sauce generously over the top.\n4. Garnish with fried shallots and serve with crackers.\n    `,
  "nutrition": { calories: "480 kcal", carbohydrates: "60g", protein: "15g", fat: "20g" },
  "healthBenefits": "A highly satisfying vegetarian meal. Tofu provides complete plant protein. Garlic is great for the cardiovascular system.",
  "excessRisks": "It is very carb-heavy (rice cake + noodles) and calorie-dense from the peanut sauce and sweet soy sauce."
},
  {
  "id": "indo-nasi-uduk",
  "name": "Nasi Uduk",
  "desc": "Jakarta style steamed rice cooked in coconut milk, spiced with lemongrass, cloves, and pandan leaves.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Nasi%20Uduk?width=600&height=400&nologo=true",
  "emoji": "🍚",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Rice",
      "name": "Oryza sativa"
    },
    {
      "ingredient": "Pandan",
      "name": "Pandanus amaryllifolius"
    },
    {
      "ingredient": "Clove",
      "name": "Syzygium aromaticum"
    }
  ],
  "recipe": `\n### Ingredients\n* 2 cups white rice\n* 400ml thin coconut milk\n* 2 pandan leaves, tied in a knot\n* 1 lemongrass stalk, bruised\n* 2 cloves, 1 bay leaf\n* 1 tsp salt\n\n### Instructions\n1. Wash the rice and drain.\n2. In a rice cooker or pot, combine the rice, coconut milk, pandan, lemongrass, cloves, bay leaf, and salt.\n3. Cook until the rice is fully done and the liquid is absorbed.\n4. Fluff gently.\n5. Serve with fried shallots, sliced omelet, fried chicken, and peanut sauce.\n    `,
  "nutrition": { calories: "350 kcal", carbohydrates: "45g", protein: "5g", fat: "15g" },
  "healthBenefits": "Pandan and cloves contain natural essential oils with mild relaxing and antioxidant effects.",
  "excessRisks": "Like other coconut rice dishes, adding high-fat side dishes will make the overall meal extremely high in saturated fats and total calories."
},
  {
  "id": "indo-lontong-sayur",
  "name": "Lontong Sayur",
  "desc": "Rice cakes served in a vegetable stew of coconut milk flavored with spices.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Lontong%20Sayur?width=600&height=400&nologo=true",
  "emoji": "🍲",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chayote",
      "name": "Sechium edule"
    },
    {
      "ingredient": "Long Bean",
      "name": "Vigna unguiculata subsp. sesquipedalis"
    },
    {
      "ingredient": "Rice (Lontong)",
      "name": "Oryza sativa"
    }
  ],
  "recipe": `\n### Ingredients\n* 2 pieces of Lontong (compressed rice cake), sliced\n* 1 chayote, julienned\n* 100g long beans, cut into 1-inch pieces\n* 500ml coconut milk\n* **Spice Paste**: Red chilies, shallots, garlic, candlenut, turmeric, galangal.\n\n### Instructions\n1. Sauté the spice paste until fragrant.\n2. Add the julienned chayote and long beans; stir briefly.\n3. Pour in the coconut milk. Add salt and a pinch of sugar.\n4. Simmer until the vegetables are tender but retain some crunch.\n5. Place sliced lontong in a bowl, ladle the vegetable stew over it, and serve with boiled egg or tofu.\n    `,
  "nutrition": { calories: "400 kcal", carbohydrates: "45g", protein: "8g", fat: "20g" },
  "healthBenefits": "Chayote and long beans provide dietary fiber, folate, and vitamin C. Good for digestion.",
  "excessRisks": "Coconut milk broth if consumed entirely adds a lot of saturated fats. Often eaten with crackers, adding empty calories."
},
  {
  "id": "indo-semur-daging",
  "name": "Semur Daging",
  "desc": "Indonesian meat stew braised in thick, sweet soy sauce, shallots, and nutmeg.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Semur%20Daging?width=600&height=400&nologo=true",
  "emoji": "🥩",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Beef",
      "name": "Bos taurus"
    },
    {
      "ingredient": "Nutmeg",
      "name": "Myristica fragrans"
    },
    {
      "ingredient": "Soybean",
      "name": "Glycine max"
    }
  ],
  "recipe": `\n### Ingredients\n* 500g beef (chuck), thinly sliced or cubed\n* 2 potatoes, peeled and cut into chunks\n* 4-5 tbsp sweet soy sauce (Kecap Manis)\n* 1/2 tsp ground nutmeg, 2 cloves\n* **Spice Paste**: Shallots, garlic, ginger.\n* Water or beef broth.\n\n### Instructions\n1. Sauté the spice paste, cloves, and nutmeg until fragrant.\n2. Add the beef and cook until browned on the outside.\n3. Pour in water/broth and sweet soy sauce. Bring to a boil.\n4. Lower the heat and simmer for 1.5 hours until the meat is halfway tender.\n5. Add the potatoes and continue simmering until both meat and potatoes are fully tender and the sauce has thickened.\n    `,
  "nutrition": { calories: "450 kcal", carbohydrates: "25g", protein: "30g", fat: "22g" },
  "healthBenefits": "Nutmeg is known for soothing indigestion and its anti-inflammatory properties. Beef provides high-quality iron and protein.",
  "excessRisks": "Sweet soy sauce contains massive amounts of added sugar. Frequent consumption can elevate blood glucose levels rapidly."
},
  {
  "id": "indo-sop-buntut",
  "name": "Sop Buntut",
  "desc": "Oxtail soup in a rich but clear beef broth, featuring boiled potatoes, carrots, and tomatoes.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Sop%20Buntut?width=600&height=400&nologo=true",
  "emoji": "🍲",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Beef (Oxtail)",
      "name": "Bos taurus"
    },
    {
      "ingredient": "Carrot",
      "name": "Daucus carota"
    },
    {
      "ingredient": "Nutmeg",
      "name": "Myristica fragrans"
    }
  ],
  "recipe": `\n### Ingredients\n* 1kg oxtail, cut into joints\n* 2 carrots, sliced\n* 2 potatoes, cubed\n* 1 tomato, cut into wedges\n* **Spices**: Nutmeg, cloves, cinnamon stick, garlic, shallots.\n* Scallions and celery leaves.\n\n### Instructions\n1. Blanch the oxtail in boiling water for 10 minutes, then discard the water to clean it.\n2. In a clean pot, boil the oxtail with fresh water, cinnamon, cloves, and salt until very tender (2-3 hours).\n3. Sauté crushed garlic, shallots, and freshly grated nutmeg, then add to the broth.\n4. Add potatoes and carrots in the last 15 minutes of cooking.\n5. Add tomatoes right before turning off the heat. Garnish with scallions and celery.\n    `,
  "nutrition": { calories: "480 kcal", carbohydrates: "20g", protein: "35g", fat: "28g" },
  "healthBenefits": "Oxtail is rich in gelatin and collagen, which is excellent for joint health, skin elasticity, and gut lining.",
  "excessRisks": "Oxtail is a very fatty cut of meat. The broth can become excessively high in saturated fat and cholesterol if not skimmed properly."
},
  {
  "id": "indo-tongseng",
  "name": "Tongseng",
  "desc": "A rich, spicy goat meat stew cooked with coconut milk, sweet soy sauce, and cabbage.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Tongseng?width=600&height=400&nologo=true",
  "emoji": "🍲",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Goat",
      "name": "Capra hircus"
    },
    {
      "ingredient": "Cabbage",
      "name": "Brassica oleracea var. capitata"
    },
    {
      "ingredient": "Chili",
      "name": "Capsicum annuum"
    }
  ],
  "recipe": `\n### Ingredients\n* 400g goat meat, diced\n* 200g cabbage, roughly chopped\n* 1 tomato, cut into wedges\n* 200ml thin coconut milk\n* 2 tbsp sweet soy sauce\n* **Spice Paste**: Shallots, garlic, coriander, turmeric, ginger, bird's eye chilies.\n\n### Instructions\n1. Sauté the spice paste until fragrant.\n2. Add the goat meat and cook until nicely browned and coated with spices.\n3. Pour in the coconut milk, add sweet soy sauce, bring to a boil, then simmer until the meat is tender.\n4. Once the meat is soft, stir in the cabbage and tomato wedges.\n5. Cook for another 2-3 minutes until the vegetables are wilted but still crunchy. Serve hot.\n    `,
  "nutrition": { calories: "550 kcal", carbohydrates: "18g", protein: "35g", fat: "35g" },
  "healthBenefits": "Goat meat is a great source of lean protein and essential minerals like zinc and iron.",
  "excessRisks": "The combination of coconut milk and sweet soy sauce makes it very calorie-rich. For some, eating too much goat meat can trigger slight blood pressure surges."
}
  ,
  {
  "id": "indo-martabak-manis",
  "name": "Martabak Manis",
  "desc": "A thick, sweet pancake heavily stuffed with chocolate, cheese, peanuts, and condensed milk.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Martabak%20Manis?width=600&height=400&nologo=true",
  "emoji": "🥞",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Wheat",
      "name": "Triticum"
    },
    {
      "ingredient": "Peanut",
      "name": "Arachis hypogaea"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Mix flour, yeast, sugar, eggs, and water.
2. Pour into a heated thick pan, let it rise, sprinkle sugar, and fold with toppings.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High calorie.
  "healthBenefits": "Energy boost.",
  "excessRisks": "High sugar and fat."
},
  {
  "id": "indo-martabak-telur",
  "name": "Martabak Telur",
  "desc": "Savory folded crisp pancake stuffed with seasoned eggs, meat, and green onions.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Martabak%20Telur?width=600&height=400&nologo=true",
  "emoji": "🍳",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chicken (Egg)",
      "name": "Gallus gallus domesticus"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Fill a thin stretched dough with a mix of whisked eggs, minced meat, and scallions.
2. Fold and deep fry.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High protein, high fat.
  "healthBenefits": "Good protein source.",
  "excessRisks": "High cholesterol and fat."
},
  {
  "id": "indo-klepon",
  "name": "Klepon",
  "desc": "Sweet rice cake balls filled with liquid palm sugar and coated in grated coconut.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Klepon?width=600&height=400&nologo=true",
  "emoji": "🍡",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Glutinous Rice",
      "name": "Oryza sativa glutinosa"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Mix glutinous rice flour with pandan juice, fill with palm sugar, boil until they float, then roll in grated coconut.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High carbs.
  "healthBenefits": "Quick energy.",
  "excessRisks": "High sugar."
},
  {
  "id": "indo-onde-onde",
  "name": "Onde-Onde",
  "desc": "Fried sesame seed balls filled with sweet mung bean paste.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Onde-Onde?width=600&height=400&nologo=true",
  "emoji": "🧆",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Sesame",
      "name": "Sesamum indicum"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Make dough from glutinous rice flour, fill with mung bean paste, coat with sesame seeds and deep fry.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate calorie.
  "healthBenefits": "Good fats from sesame.",
  "excessRisks": "High oil content from deep frying."
},
  {
  "id": "indo-pisang-goreng",
  "name": "Pisang Goreng",
  "desc": "Deep-fried banana or plantain coated in batter.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Pisang%20Goreng?width=600&height=400&nologo=true",
  "emoji": "🍌",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Banana",
      "name": "Musa"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Coat sliced bananas in a batter of flour, sugar, and water.
2. Deep fry until golden.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate calories.
  "healthBenefits": "Potassium from bananas.",
  "excessRisks": "High saturated fat from frying."
},
  {
  "id": "indo-kue-lapis",
  "name": "Kue Lapis",
  "desc": "Colorful layered steamed soft rice pudding cake.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Kue%20Lapis?width=600&height=400&nologo=true",
  "emoji": "🍰",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Tapioca",
      "name": "Manihot esculenta"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Make a batter with tapioca, rice flour, and coconut milk.
2. Divide into colors and steam layer by layer.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High carbs.
  "healthBenefits": "Gluten free.",
  "excessRisks": "High sugar."
},
  {
  "id": "indo-lupis",
  "name": "Lupis",
  "desc": "Triangular sticky rice cake served with grated coconut and palm sugar syrup.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Lupis?width=600&height=400&nologo=true",
  "emoji": "🍙",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Glutinous Rice",
      "name": "Oryza sativa glutinosa"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Wrap glutinous rice in banana leaves in a triangle shape, boil until cooked.
2. Serve with coconut and sugar syrup.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High carbs.
  "healthBenefits": "Quick energy.",
  "excessRisks": "High sugar."
},
  {
  "id": "indo-risoles",
  "name": "Risoles",
  "desc": "Savory fried pastry rolls filled with vegetables and chicken, coated in breadcrumbs.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Risoles?width=600&height=400&nologo=true",
  "emoji": "🌯",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Carrot",
      "name": "Daucus carota"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Make thin crepes, fill with ragout (carrots, chicken), fold, coat in egg and breadcrumbs, and fry.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate calorie.
  "healthBenefits": "Vegetables provide fiber.",
  "excessRisks": "Fried food risks."
},
  {
  "id": "indo-lemper",
  "name": "Lemper",
  "desc": "Sticky rice wrapper filled with savory spiced shredded chicken, wrapped in banana leaf.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Lemper?width=600&height=400&nologo=true",
  "emoji": "🍙",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chicken",
      "name": "Gallus gallus domesticus"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Cook glutinous rice with coconut milk.
2. Flatten, add cooked shredded chicken, roll, and wrap in banana leaves.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Balanced carbs and protein.
  "healthBenefits": "Good protein source.",
  "excessRisks": "High calories from coconut milk."
},
  {
  "id": "indo-serabi",
  "name": "Serabi",
  "desc": "Traditional Indonesian pancake made from rice flour and coconut milk or shredded coconut.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Serabi?width=600&height=400&nologo=true",
  "emoji": "🥞",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Coconut",
      "name": "Cocos nucifera"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Pour a liquid batter of rice flour and coconut milk into a clay pan over charcoal.
2. Serve with sugar syrup.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate calorie.
  "healthBenefits": "Dairy-free.",
  "excessRisks": "High sugar syrup."
},
  {
  "id": "indo-dadar-gulung",
  "name": "Dadar Gulung",
  "desc": "Green pandan crepe rolled with sweet grated coconut filling.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Dadar%20Gulung?width=600&height=400&nologo=true",
  "emoji": "🌯",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Pandan",
      "name": "Pandanus amaryllifolius"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Make a green crepe using pandan juice.
2. Fill with unti (grated coconut cooked with palm sugar) and fold.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate calories.
  "healthBenefits": "Fiber from coconut.",
  "excessRisks": "High sugar."
},
  {
  "id": "indo-getuk",
  "name": "Getuk",
  "desc": "Cassava-based sweet snack, often colored and served with grated coconut.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Getuk?width=600&height=400&nologo=true",
  "emoji": "🍠",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Cassava",
      "name": "Manihot esculenta"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Boil cassava, mash it with sugar and food coloring, then shape it.
2. Serve with freshly grated coconut.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High carbs.
  "healthBenefits": "Good source of complex carbs.",
  "excessRisks": "High sugar."
},
  {
  "id": "indo-wingko-babat",
  "name": "Wingko Babat",
  "desc": "Chewy traditional coconut pancake made of glutinous rice flour and grated coconut.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Wingko%20Babat?width=600&height=400&nologo=true",
  "emoji": "🥞",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Coconut",
      "name": "Cocos nucifera"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Mix glutinous rice flour, grated young coconut, and sugar.
2. Bake or grill in small round shapes.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate calorie.
  "healthBenefits": "Good source of energy.",
  "excessRisks": "High sugar and fat."
},
  {
  "id": "indo-pastel",
  "name": "Pastel",
  "desc": "Fried pastry filled with glass noodles, carrots, and boiled egg.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Pastel?width=600&height=400&nologo=true",
  "emoji": "🥟",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Carrot",
      "name": "Daucus carota"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Make a pastry crust.
2. Fill with sautéed carrots, vermicelli, and egg slices.
3. Pinch the edges and deep fry.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate calorie.
  "healthBenefits": "Vegetable filling provides vitamins.",
  "excessRisks": "High fat from pastry and frying."
},
  {
  "id": "indo-cireng",
  "name": "Cireng",
  "desc": "Chewy fried tapioca dough snack, often served with a spicy dip.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Cireng?width=600&height=400&nologo=true",
  "emoji": "🧆",
  "country": "Indonesia",
  "style": "Traditional",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Tapioca",
      "name": "Manihot esculenta"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Mix tapioca flour with water, garlic, and scallions to form a sticky dough.
2. Flatten and deep fry.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High carbs.
  "healthBenefits": "Quick energy.",
  "excessRisks": "High calorie from frying and empty carbs."
},
  {
  "id": "indo-ayam-geprek",
  "name": "Ayam Geprek",
  "desc": "Crispy fried chicken crushed and mixed with hot and spicy sambal.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Ayam%20Geprek?width=600&height=400&nologo=true",
  "emoji": "🍗",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chicken",
      "name": "Gallus gallus domesticus"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Fry battered chicken until crispy.
2. Crush out flat using a pestle and intensely mix with raw garlic chili sambal.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High protein, high fat.
  "healthBenefits": "Protein and capsaicin boost metabolism.",
  "excessRisks": "Very spicy, can cause gastric issues."
},
  {
  "id": "indo-sate-taichan",
  "name": "Sate Taichan",
  "desc": "A modern spin on satay: white grilled chicken meat served without peanut sauce, very spicy.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Sate%20Taichan?width=600&height=400&nologo=true",
  "emoji": "🍢",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chicken",
      "name": "Gallus gallus domesticus"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Grill unmarinated or lightly salted chicken skewers.
2. Serve with a watery, extremely spicy chili sauce and lime.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High protein, low fat.
  "healthBenefits": "Lean protein.",
  "excessRisks": "Spiciness can upset the stomach."
},
  {
  "id": "indo-seblak",
  "name": "Seblak",
  "desc": "A savory and spicy dish made of wet krupuk (crackers) cooked with flavorings and aromatic root.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Seblak?width=600&height=400&nologo=true",
  "emoji": "🍲",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Lesser Galangal",
      "name": "Kaempferia galanga"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Boil raw crackers until soft.
2. Sauté a paste of lesser galangal, garlic, and chilies.
3. Add water, crackers, egg, and toppings.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High carbs.
  "healthBenefits": "Warming spices.",
  "excessRisks": "High sodium and empty carbs."
},
  {
  "id": "indo-nasi-kulit-crispy",
  "name": "Nasi Kulit Crispy",
  "desc": "Rice served with deeply fried, ultra-crispy seasoned chicken skin.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Nasi%20Kulit%20Crispy?width=600&height=400&nologo=true",
  "emoji": "🍛",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chicken Skin",
      "name": "Gallus gallus domesticus"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Clean chicken skin, marinate, coat in seasoned flour, and deep fry until crispy.
2. Serve with warm rice and sambal.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High fat, high calorie.
  "healthBenefits": "Comfort food.",
  "excessRisks": "Extremely high saturated fat and cholesterol."
},
  {
  "id": "indo-indomie-kreasi",
  "name": "Indomie Goreng Kreasi",
  "desc": "Next-level instant noodles topped with salted egg, mozzarella, or extravagant meats.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Indomie%20Goreng%20Kreasi?width=600&height=400&nologo=true",
  "emoji": "🍜",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Wheat",
      "name": "Triticum"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Cook instant noodles as instructed.
2. Add extravagant toppings like torch-melted mozzarella, corned beef, and a soft-boiled egg.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High carbs and fat.
  "healthBenefits": "Quick energy.",
  "excessRisks": "Very high sodium and preservatives."
},
  {
  "id": "indo-nasi-goreng-seafood-premium",
  "name": "Nasi Goreng Seafood Premium",
  "desc": "Upgraded fried rice with premium seafood like squid, large prawns, and luxurious sauces.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Nasi%20Goreng%20Seafood%20Premium?width=600&height=400&nologo=true",
  "emoji": "🍛",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Shrimp",
      "name": "Caridea"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Sauté premium seafood, add day-old rice, and mix with a rich blend of oyster sauce, soy sauce, and spices.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High protein.
  "healthBenefits": "Seafood provides omega-3 and minerals.",
  "excessRisks": "High cholesterol from certain seafood."
},
  {
  "id": "indo-korean-fusion",
  "name": "Korean-Indonesian Fusion",
  "desc": "Dishes like Tteokbokki mixed with Seblak or spicy Gochujang Ayam Geprek.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Korean-Indonesian%20Fusion?width=600&height=400&nologo=true",
  "emoji": "🍲",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chili",
      "name": "Capsicum annuum"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Combine Korean gochujang and rice cakes with Indonesian aromatic roots and sambals.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High carbs.
  "healthBenefits": "Fusion of different spices.",
  "excessRisks": "High sodium."
},
  {
  "id": "indo-croffle-pandan",
  "name": "Croffle Pandan",
  "desc": "A cross between a croissant and a waffle, infused with pandan flavor and syrup.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Croffle%20Pandan?width=600&height=400&nologo=true",
  "emoji": "🧇",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Pandan",
      "name": "Pandanus amaryllifolius"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Take croissant dough infused with pandan, roll it in sugar, and cook it in a waffle maker until caramelized.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High fat, high sugar.
  "healthBenefits": "Pandan offers relaxation.",
  "excessRisks": "High sugar and saturated fat."
},
  {
  "id": "indo-dimsum-kekinian",
  "name": "Dimsum Kekinian",
  "desc": "Modern dimsum topped with mentai sauce, mozzarella, or salted egg sauce, blowtorched.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Dimsum%20Kekinian?width=600&height=400&nologo=true",
  "emoji": "🥟",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chicken",
      "name": "Gallus gallus domesticus"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Steam chicken dimsum, coat generously with mayo-pollack roe sauce (mentai), and torch it until charred.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High fat, moderate protein.
  "healthBenefits": "Good protein.",
  "excessRisks": "Very high calories from mayonnaise sauces."
},
  {
  "id": "indo-batagor-kuah",
  "name": "Batagor Kuah",
  "desc": "Modern take on Batagor, served with warm, savory broth instead of peanut sauce.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Batagor%20Kuah?width=600&height=400&nologo=true",
  "emoji": "🍲",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Mackerel",
      "name": "Scomberomorus"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Deep fry fish dumplings (Batagor).
2. Serve in a hot bowl of savory chicken or beef broth with chili.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate calories.
  "healthBenefits": "Hydrating broth, fish protein.",
  "excessRisks": "Fried dough."
},
  {
  "id": "indo-mie-pedas-level",
  "name": "Mie Pedas Level",
  "desc": "Trendy dry noodles featuring extreme spiciness levels ranging from level 1 to 100.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Mie%20Pedas%20Level?width=600&height=400&nologo=true",
  "emoji": "🍜",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chili",
      "name": "Capsicum annuum"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Boil noodles and mix thoroughly with pureed bird's eye chilies, chili oil, and savory chicken seasoning.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High carbs.
  "healthBenefits": "Capsaicin.",
  "excessRisks": "Can severely irritate the digestive tract."
},
  {
  "id": "indo-ayam-bakar-taliwang-modern",
  "name": "Ayam Bakar Taliwang Modern",
  "desc": "Lombok's spicy grilled chicken served in modern rice bowl setups.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Ayam%20Bakar%20Taliwang%20Modern?width=600&height=400&nologo=true",
  "emoji": "🍗",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Chicken",
      "name": "Gallus gallus domesticus"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Marinate chicken in terasi (shrimp paste) and chilies, grill, and serve over rice with modern aesthetic garnishes.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High protein.
  "healthBenefits": "Lean meat if skin is removed.",
  "excessRisks": "Charred parts."
},
  {
  "id": "indo-boba-toast",
  "name": "Boba Toast",
  "desc": "Thick, fluffy toast slathered in cream cheese or milk tea cream and topped with chewy tapioca pearls.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Boba%20Toast?width=600&height=400&nologo=true",
  "emoji": "🍞",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Food",
  "scientificNames": [
    {
      "ingredient": "Tapioca",
      "name": "Manihot esculenta"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Toast thick bread, cover with sweet rich cream, and top with cooked warm boba pearls.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Very high carb and sugar.
  "healthBenefits": "Comfort food.",
  "excessRisks": "Sugar spike."
},
  {
  "id": "drink-boba-milk-tea",
  "name": "Boba Milk Tea",
  "desc": "Classic sweet milk tea with chewy tapioca pearls.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Boba%20Milk%20Tea?width=600&height=400&nologo=true",
  "emoji": "🧋",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Beverage",
  "scientificNames": [
    {
      "ingredient": "Tea",
      "name": "Camellia sinensis"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Brew black tea, mix with milk and sugar syrup, pour over ice and cooked tapioca pearls.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High sugar.
  "healthBenefits": "Antioxidants from tea.",
  "excessRisks": "Excessive sugar."
},
  {
  "id": "drink-brown-sugar-latte",
  "name": "Brown Sugar Latte",
  "desc": "Fresh milk poured over a rich, tiger-striped syrup of molten palm sugar and boba.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Brown%20Sugar%20Latte?width=600&height=400&nologo=true",
  "emoji": "🧋",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Beverage",
  "scientificNames": [
    {
      "ingredient": "Cow's Milk",
      "name": "Bos taurus"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Simmer tapioca pearls in dark brown sugar syrup.
2. Swirl on a cup's edges, fill with ice and fresh milk.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High sugar and dairy.
  "healthBenefits": "Calcium from milk.",
  "excessRisks": "High sugar."
},
  {
  "id": "drink-es-kepal-milo",
  "name": "Es Kepal Milo",
  "desc": "Huge ball of shaved ice drenched in a thick, rich Milo (chocolate malt) syrup.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Es%20Kepal%20Milo?width=600&height=400&nologo=true",
  "emoji": "🍧",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Beverage",
  "scientificNames": [
    {
      "ingredient": "Cocoa",
      "name": "Theobroma cacao"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Shape shaved ice into a ball.
2. Make a very thick syrup with Milo powder and condensed milk, pour over the ice.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Extremely high sugar.
  "healthBenefits": "Cooling dessert.",
  "excessRisks": "Risk of diabetes."
},
  {
  "id": "drink-matcha-latte-lokal",
  "name": "Matcha Latte Lokal",
  "desc": "Matcha mixed with locally sourced milk and sometimes topped with cheese foam.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Matcha%20Latte%20Lokal?width=600&height=400&nologo=true",
  "emoji": "🍵",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Beverage",
  "scientificNames": [
    {
      "ingredient": "Matcha",
      "name": "Camellia sinensis"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Whisk matcha in warm water, pour over ice and milk.
2. Top with a creamy sweet/salty cheese foam.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate to high calorie.
  "healthBenefits": "Antioxidants.",
  "excessRisks": "High fat from cheese foam."
},
  {
  "id": "drink-kopi-dalgona",
  "name": "Kopi Dalgona",
  "desc": "Whipped instant coffee foam over cold milk.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Kopi%20Dalgona?width=600&height=400&nologo=true",
  "emoji": "☕",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Beverage",
  "scientificNames": [
    {
      "ingredient": "Coffee",
      "name": "Coffea"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Whip equal parts instant coffee, sugar, and hot water until stiff peaks form.
2. Spoon over iced milk.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High sugar.
  "healthBenefits": "Caffeine boost.",
  "excessRisks": "High sugar."
},
  {
  "id": "drink-thai-tea-fusion",
  "name": "Thai Tea Fusion",
  "desc": "Strongly brewed Ceylon tea with condensed milk, mixed with modern toppings.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Thai%20Tea%20Fusion?width=600&height=400&nologo=true",
  "emoji": "🧋",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Beverage",
  "scientificNames": [
    {
      "ingredient": "Tea",
      "name": "Camellia sinensis"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Brew Thai tea mix, stir in condensed and evaporated milk, serve over crushed ice.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High sugar.
  "healthBenefits": "Cooling.",
  "excessRisks": "High sugar."
},
  {
  "id": "drink-taro-latte",
  "name": "Taro Latte",
  "desc": "Sweet, purple-colored milky drink flavor derived from taro root.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Taro%20Latte?width=600&height=400&nologo=true",
  "emoji": "🍠",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Beverage",
  "scientificNames": [
    {
      "ingredient": "Taro",
      "name": "Colocasia esculenta"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Mix taro powder with hot water, add simple syrup, and top with ice and fresh milk.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: High carbs.
  "healthBenefits": "Comforting.",
  "excessRisks": "Usually contains artificial flavorings."
},
  {
  "id": "drink-sparkling-pandan",
  "name": "Sparkling Pandan Drink",
  "desc": "Carbonated soda mixed with natural pandan extract and lime.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Sparkling%20Pandan%20Drink?width=600&height=400&nologo=true",
  "emoji": "🍹",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Beverage",
  "scientificNames": [
    {
      "ingredient": "Pandan",
      "name": "Pandanus amaryllifolius"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Boil pandan leaves with sugar for syrup.
2. Mix syrup with sparkling water, ice, and a squeeze of lime.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate sugar.
  "healthBenefits": "Refreshing, relaxing scent.",
  "excessRisks": "Acidic and sugary."
},
  {
  "id": "drink-es-kopi-susu-gula-aren",
  "name": "Es Kopi Susu Gula Aren",
  "desc": "Iced espresso and milk sweetened exclusively with Indonesian palm sugar.",
  "image": "https://image.pollinations.ai/prompt/Delicious%20high%20quality%20food%20photography%20professional%20culinary%20shot%20of%20Es%20Kopi%20Susu%20Gula%20Aren?width=600&height=400&nologo=true",
  "emoji": "☕",
  "country": "Indonesia",
  "style": "Modern",
  "category": "Beverage",
  "scientificNames": [
    {
      "ingredient": "Coffee",
      "name": "Coffea"
    },
    {
      "ingredient": "Palm Sugar",
      "name": "Arenga pinnata"
    }
  ],
  "recipe": `
### Ingredients
* Full ingredients listed below/above.

### Instructions
1. Pull a shot of espresso.
2. Add a generous pump of liquid palm sugar, ice, and top with fresh milk.
`,
  "nutrition": { calories: "Unknown", carbohydrates: "Unknown", protein: "Unknown", fat: "Unknown" }, // Original: Moderate calories.
  "healthBenefits": "Caffeine focus. Palm sugar has slightly lower glycemic index.",
  "excessRisks": "Caffeine tolerance issues."
}
];

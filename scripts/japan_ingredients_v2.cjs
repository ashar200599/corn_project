...
for (const [id, ingArray] of Object.entries(jpIngredients)) {
  // Be careful if some ids exist multiple times (e.g. jp-sushi)
  // We need to parse the file string and replace the recipe property.
  const idRegex = new RegExp(`"id":\\s*"${id}"([\\s\\S]*?)"recipe":\\s*"([^"]+)"`, 'g');
  
  dStr = dStr.replace(idRegex, (match, middle, recipeStr) => {
    // If recipeStr already has multiple lines of ingredients, let it be
    const idx = recipeStr.indexOf('### Instructions');
    if (idx !== -1) {
      const ingSection = recipeStr.substring(0, idx);
      const lines = ingSection.split('\\n').filter(l => l.trim().length > 0 && !l.includes('###'));
      if (lines.length >= 3) {
        return match;
      }
    }
    
    let newRecipeStr = `### Ingredients\\n${ingArray.join('\\n')}\\n\\n`;
    if (idx !== -1) {
      newRecipeStr += recipeStr.substring(idx);
    } else {
      newRecipeStr += `### Instructions\\n1. Prepare according to traditional Japanese recipe.`;
    }
    
    return `"id": "${id}"${middle}"recipe": "${newRecipeStr}"`;
  });
}

fs.writeFileSync('src/data/dishes.ts', dStr, 'utf8');
console.log("Re-applied ingredients.");
